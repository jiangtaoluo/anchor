    ...
    for line in ifile:
        if line[0] != '#':  # skip the comment line beginning with '#'
            ...
            if num == 1:
                ...
                Sum = list(map(add, Sum, record)) #每一列都加起来，形成一个list

                Min = [min(x) for x in list(zip(Min, record))] #迭代后求出每一列的最小值
                Max = [max(x) for x in list(zip(Max, record))]#迭代后求出每一列的最大值
        else:
            header += line
print(header)
